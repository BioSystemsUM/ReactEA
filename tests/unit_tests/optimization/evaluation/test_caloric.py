...
from reactea.optimization.evaluation import Caloric
from .test_evaluation_functions import EvaluationFunctionBaseTestCase


class TestCaloric(EvaluationFunctionBaseTestCase, TestCase):

    def test_evaluation_function(self):
        mols = self.mols[-1]
        spds = Caloric(maximize=True, worst_fitness=0.0)
        scores = spds.get_fitness(mols)

        self.assertEqual(len(scores), 1)
        self.assertEqual(spds.method_str(), "Caloric")
